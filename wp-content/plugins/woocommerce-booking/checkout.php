		...
		if ( isset( $booking_data[ 'hidden_date' ] ) && '' != $booking_data[ 'hidden_date' ] ) {
			...
			if ( isset ( $booking_settings['booking_enable_multiple_day'] ) && $booking_settings['booking_enable_multiple_day'] != 'on'  ) {
				if( $called_from != 'admin' ) {
					...
					if ( isset ( $booking_data['time_slot'] ) && $booking_data['time_slot'] != "" && ( array_key_exists ( 'date', $booking_data ) || array_key_exists ( 'booking_date', $booking_data ) ) ) {
						if ( isset ( $query_to_time ) && $query_to_time != '' ) {
					...
					foreach ( $order_results as $k => $v ) {
						...
						bkap_checkout::bkap_update_booking_order_history( $order_id, $booking_id, $update_or_insert );
						$j++;
					}
				}
			}
		...
		foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
			...
			if( $type_of_slot == 'multiple' ) {
				...
				if ( isset( $values['bkap_booking'] ) && isset( $values['bkap_booking'][0]['date'] ) ) :
					...
					do_action( 'after_bkap_create_booking_post', $results[0]->order_item_id, $post_id, $quantity, $booking_data, $variation_id );
