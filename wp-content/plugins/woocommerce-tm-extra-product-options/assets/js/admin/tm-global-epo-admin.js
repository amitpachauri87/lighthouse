    ...
    $.tmEPOAdmin = {
            ...
            if (!product_type.length){
                ...
                if (product_type.val()=="variable"){
                    ...
                    if (variation_element.length){
                        ...
                        variation_element_builder_wrapper.find(".icon.clone,.icon.builder_add_on_section,.icon.size,.icon.move,.icon.plus,.icon.minus").remove();
                        ...
                        //variation_element_builder_wrapper.find("."+_rlogictab.find(".tab-header").attr("data-id")).remove();
                        _rlogictab.remove();
                        
                        //variation_element_builder_wrapper.find(".bitem_wrapper").sortable( "destroy" );                        

                        $.tmEPOAdmin.var_is("tm-style-variation-added",true);

                        variation_element.addClass("tma-nomove");
                        variation_element.find(".icon.size,.icon.clone,.icon.move,.icon.plus,.icon.minus,.icon.delete").remove();
                        _rlogictab=variation_element.find(".tma-tab-logic,.tma-tab-css");
                        //variation_element.find("."+_rlogictab.find(".tab-header").attr("data-id")).remove();
                        _rlogictab.remove();            
        ...
        builder_add_variation_onClick: function(e) {
            ...
            if (_template) {
                ...
                if (_clone) {
                    ...
                    _clone.find(".icon.clone,.icon.builder_add_on_section,.icon.size,.icon.move,.icon.plus,.icon.minus").remove();
                    ...
                    _clone2.find(".icon.size,.icon.clone,.icon.move,.icon.plus,.icon.minus,.icon.delete").remove();
        ...
        element_logic_init: function(el){
            ...
            sections.each(function(i,section){
                ...
                var fields=$(section).find($.tmEPOAdmin.can_take_logic()).not($(el));
                var values=[];                               
        ...
        validate_rule: function(rule,bitem){
            if ( !("element" in rule) || !("operator" in rule) || !("section" in rule) || !("value" in rule) ){
        ...
        removeTinyMCE: function(element) {
            ...
            $(element).find('textarea').each(function(i, textarea) {
                ...
                if (id &&  tinyMCE && tinyMCE.editors) {
                    ...
                    var is_tinymce_active = (typeof tinyMCE != "undefined") && tinyMCE.editors[id] && !tinyMCE.editors[id].isHidden();
        ...
        set_fields_change: function(obj) {
            ...
            obj.each(function(i,el){
                var btxpd = $(this).closest(".builder_element_wrap").find(".builder_textfield_price_div");
                    //btchad = $(this).closest(".builder_element_wrap").find(".builder_textfield_hide_amount_div");

                if ($(this).val()=="currentstep"){
                    if ($(btxpd).length)btxpd.hide();
                    //if ($(btchad).length)btchad.hide();
                }else{
                    if ($(btxpd).length)btxpd.show();
                    //if ($(btchad).length)btchad.show();
                }
            });  
        },
        ...
        variations_display_as: function (e){
            ...
            $this.each(function(i,el){
                ...
                if (selected_mode=="select"){
                    tm_attribute.find(".tma-hide-for-select-box").hide();
                }else{
                    tm_attribute.find(".tma-hide-for-select-box").show();
                }
                if (selected_mode=="image" || selected_mode=="color"){
                    tm_attribute.find(".tma-show-for-swatches").show();
                }else{
                    tm_attribute.find(".tma-show-for-swatches").hide();
                }
