...
// Date picker setup
    ...
    function tm_variation_check_match(element, val2, operator){
        if (element!=null && val1!=null){
            val2 = val2 ? parseInt(val2) : -1;
        }
        var variations_form=$(element).closest(".variations_form"),
            val1=variations_form.find( 'input[name="variation_id"]' ).val();
        switch(operator){
        case "is" :
    ...
    function tm_init_epo(){
         ...
         */
        $(".tm-extra-product-options").on('change', 'input.tm-epo-field.tmcp-checkbox', function () {            
        ...
        function tm_epo_rules($thecart) {
            ...
            all_carts.each(function(cart_index,cart){
                ...
                var per_product_pricing=true,
                    bto = $(this).closest('.bto_item,.component.product'),
                    current_variation=cart.find('input[name^=variation_id]').val(),
                    is_bto=false,
                    ...
                    if (typeof rules === "object") {
                        ...
                        $(element).find('.tmcp-field').each(function(i, e) {
                            if (per_product_pricing){
                                $(e).data('price', tm_set_tax_price(price,$totals));
        ...
        function tm_epo_init($form,$formcart) {
            ...
            .off('change.cpf tm_epo_variation_element_change')
            .on('change.cpf tm_epo_variation_element_change', function(e) {
                var t=$(this),
            ...
            .on('tm-select-change', function() {
                ...
                var formatted_price = tm_set_price($(this).find('option:selected').data('price'), $totals_holder);
            ...
            $epo_holder.find('.tm-epo-field:not(.tm-epo-variation-element)')
            .off('change.cpf tm_trigger_product_image')
            .on('change.cpf tm_trigger_product_image',  function() {
                var $cart = $formcart || main_cart, $this=$(this);
                $cart.trigger('tm-epo-update');
                $(this).trigger('tm-select-change');
                if ($(this).is('.tm-product-image:checkbox, .tm-product-image:radio, select.tm-product-image')){
                    var uic=$(this).closest('.tmcp-field-wrap').find('label img');
                    if($(this).is('select.tm-product-image')){
                        $this=$(this).children('option:selected');
                    }
                    if ($(uic).length>0 || $(this).attr('data-image')!='' || $this.attr('data-imagep')!=''){
                        if ($(this).is(':checked') || $(this).is('select.tm-product-image')){
        ...
        function bto_support(){
            ...
            $('.bto_item,.component.product').find( '.cart' ).append('<input type="hidden" class="tm-post-support addon">');
            $('.tm-post-support.addon').on('change', function(event) {
                $(this).closest('.component.product').trigger('wc-composite-item-updated.cpf');
            });

            $('.bto_item,.component.product')
            ...
            .on( 'wc-composite-item-updated.cpf', function() {
                 ...
                 */
                cpf_section_logic(item.find(".tm-extra-product-options"));
                cpf_element_logic(item.find(".tm-extra-product-options"));
            ...
            .on( 'woocommerce-product-addons-update.cpf cpf_bto_review', function() {
                var bto_form=$(this);
                $(this).parent().find( '.bto_item,.component.product' ).each( function(){
                    var item        = $(this),
                        item_id     = item.attr('data-item-id'),
            ...
            $('.bto_item,.component.product').trigger('wc-composite-component-loaded.cpf');
        ...
        function check_bto(id){
            var show=true;
            var $totals = $('.tm-epo-totals.tm-cart-main');
            $( '.bto_form_' + id + ',#composite_form_' + id + ',#composite_data_' + id ).parent().find( '.bto_item,.component.product' ).each( function(){
                var item        = $(this),
                    item_id         = item.attr('data-item-id'),
        ...
        function tm_product_image(){
            ...
            if ($(img).length>0){
                ...
                $(window).on('tm_attempt_product_image',function(e){
                    var tm_last_visible_image_element=$('.tm-product-image:checked:visible,select.tm-product-image').last();
        ...
        if (tm_epo_js.floating_totals_box && tm_epo_js.floating_totals_box!='disable'){
            ...
            if ( main_cart && $tm_epo_totals.length ) {
                ...
                var tm_update_epo_pop = function(){
                    ...
                    $tm_floating_box.html(tm_epo_totals_html);
                }
                tm_update_epo_pop();
    ...
    function tm_custom_variations(form,item_id){
        ...
        if ($epo_holder.find('.tm-epo-variation-element').length){
            ...
            if (item_id && item_id!="main"){// on composite                
                form.find('.variations').hide().after(form.find(".tm-epo-variation-section").addClass("tm-extra-product-options"));
    ...
    var main_cart=false,
        ...
        composite_selector = '.bto_item,.component.product',
    ...
    $(document).ready(function() {
        ...
        $(document).ajaxSuccess(function() {
            var theme_flatsome=$('.product-lightbox').length;
            if(theme_flatsome){
                tm_lazyload_container=$('.product-lightbox');
                tm_init_epo();$(window).trigger("tmlazy");
            }
            return;
        });  
