...
class TM_EPO_WPML_base {
	...
	// get taxonomy terms without WMPL filters if lang is null (use to get all terms for all languages)
		...
		if ($this->is_wpml){
			...
			if (
                ...
                if (!empty($post_id)){
                	...
                	$meta_lang  = get_post_meta( $post_id ,'tm_meta_lang', true);
			...
			}elseif ( !empty($_GET['tmparentpostid']) && !empty($_GET['tmaddlang']) && ( isset($_REQUEST['action']) && $_REQUEST['action']=='add' ) ){
				if ( !empty($_GET['lang']) ){
					$url = $_SERVER['REQUEST_URI'];
	...
	public function add_lang_link($post_id, $lang, $v){
		...
		if ($this->is_wpml){
            ...
            $post_new_file = '<a title="' . $alt . '" alt="' . $alt . '" class="tmwpmllink" href="'. $post_new_file .'"><i class="fa fa-plus"></i></a>';
		}
	...
	public function edit_lang_link($post_id, $lang, $v ,$main_post_id,$noadd=false){
		...
		if ($this->is_wpml){
            ...
            $post_new_file = '<a title="' . $alt . '" alt="' . $alt . '" class="tmwpmllink" href="'.$post_new_file.'"><i class="fa fa-pencil"></i></a>';
		}
        return $post_new_file; 
	}
	...
	public function meta_box($post){
		if ($this->is_wpml){
			...
			$tmparentpostid = 0;
			$tmaddlang = '';
			$tm_meta_lang = get_post_meta( $post->ID , 'tm_meta_lang' , true );
			...
			if ( !empty($_GET['tmparentpostid']) && !empty($_GET['tmaddlang']) 
				...
				$is_original_lang = get_post_meta( $tmparentpostid , 'tm_meta_lang' , true );
			...
			if (!$is_original && empty($is_original_lang) && !empty($tm_meta_parent_post_id)){
				$is_original_lang = get_post_meta( $tm_meta_parent_post_id , 'tm_meta_lang' , true );
				if (empty($is_original_lang)){
					$is_original_lang=$this->get_default_lang();
				}  
			...
			TM_EPO_HTML()->tm_make_field( array(
					...
					"default"	=> $tmaddlang,
					"type"		=> "hidden",
					"tags"		=> array( "id"=>"tm_meta_lang", "name"=>"tm_meta_lang" )
				), 1 );
