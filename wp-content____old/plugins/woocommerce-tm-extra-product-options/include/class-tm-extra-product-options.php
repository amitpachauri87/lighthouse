...
final class TM_Extra_Product_Options {
	 ...
	 */
	 public function cart_item_price($item_price, $cart_item, $cart_item_key){
		...
		if (isset($cart_item['tm_epo_product_after_adjustment'])){
			$price = $this->get_price_for_cart($cart_item['tm_epo_product_after_adjustment'],$cart_item,"");
		}

        // Format price to display
		$price_to_display = $price;
		$original_price_to_display = $this->get_price_for_cart($cart_item['rp_wcdpd']['original_price'],$cart_item,"") ;

		$item_price = '<span class="rp_wcdpd_cart_price"><del>' . $original_price_to_display . '</del> <ins>' . $price_to_display . '</ins></span>';
            
		$item_price .='<div><span class="rp_wcdpd_cart_price tm-cart-options-prices"><ins>'.$this->get_price_for_cart($cart_item['tm_epo_options_prices'],$cart_item).'</ins></span></div>';
	...
	private function get_price_for_cart($price=0,$cart_item,$symbol=false){
		global $woocommerce;
	...
	public function cacl_fee_price($price, $product_id){
		...
		$cart 				= $woocommerce->cart;
		$taxable 			= $product->is_taxable();
		$tax_display_cart 	= get_option( 'woocommerce_tax_display_shop' );//$cart->tax_display_cart;
		$tax_string="";
		...
		if ( $this->tm_epo_hide_options_in_cart=="normal" && !empty( $cart_item['tmcartepo'] ) ) {
			...
			foreach ( $filtered_array as $section ) {
				...
				if ($this->tm_epo_hide_options_prices_in_cart=="normal"){
					$format_price=$this->get_price_for_cart($section['price'],$cart_item);
					if ($original_price!=$section['price']){
		...
		if ( ! empty( $values['tmcartepo'] ) ) {			
			...
			foreach ( $filtered_array as $section ) {
				...
				if (class_exists('WOOCS')){
					...
					$_not_woocs=$this->get_price_for_cart($section['price'],$values);
				}
	 ...
	 *
	 * IMPORTANT:
	 * We do not currently support plugins that pollute the global $woocommerce.
	 *
	 */
	...
	public function tm_epo_fields($product_id=0, $form_prefix="") {
		...
		$woo_thump_dim=get_option('shop_thumbnail_image_size');
		$css_string=".tmhexcolorimage{min-width:".$woo_thump_dim['width']."px !important;min-height:".$woo_thump_dim['height']."px !important;}";
	...
	public function frontend_scripts() {
		global $product;
		if ( ( class_exists( 'WC_Quick_View' ) && ( is_shop() || is_product_category() || is_product_tag() ) ) 
				...
				foreach ( $tmp_tmglobalprices as $price ) {
					...
					if( !has_term( $_all_categories, 'product_cat', $price) ) {
						if (TM_EPO_WPML()->is_active()){
							$price_meta_lang=get_post_meta( $price->ID, 'tm_meta_lang', true );
			...
			foreach ( $tmglobalprices as $price ) {
				...
				if ( isset( $tmcp_meta['tmfbuilder'] ) ) {
					...
					if ( is_array( $builder ) && count( $builder )>0 && isset( $builder['element_type'] ) && is_array( $builder['element_type'] ) && count( $builder['element_type'] )>0 ) {
						...
						$_sections_uniqid 	= isset($builder['sections_uniqid'])?$builder['sections_uniqid']:TM_EPO_HELPER()->tm_temp_uniqid(count( $_sections )) ;
								...
								if ( isset( $_elements[$k0] ) && isset($this->tm_original_builder_elements[$_elements[$k0]]) ) {
									...
									if ($this->tm_original_builder_elements[$_elements[$k0]]){
										if ($this->tm_original_builder_elements[$_elements[$k0]]["type"]=="single"){
											...
											if ( isset( $builder['multiple_'.$_elements[$k0].'_options_price'][$_counter[$_elements[$k0]]] ) ) {
												...
												$_imagesp=isset($builder['multiple_'.$_elements[$k0].'_options_imagep'][$_counter[$_elements[$k0]]])?$builder['multiple_'.$_elements[$k0].'_options_imagep'][$_counter[$_elements[$k0]]]:array();
									...
									if ($_elements[$k0]!="header" && $_elements[$k0]!="divider"){										
										...
										array(
											...
											'exactlimit' 		=> isset( $builder[$_prefix.'exactlimit_choices'][$_counter[$_elements[$k0]]] )?$builder[$_prefix.'exactlimit_choices'][$_counter[$_elements[$k0]]]:"",
											...
											'min_date' 			=> isset( $builder[$_prefix.'min_date'][$_counter[$_elements[$k0]]] )?$builder[$_prefix.'min_date'][$_counter[$_elements[$k0]]]:"",
											'max_date' 			=> isset( $builder[$_prefix.'max_date'][$_counter[$_elements[$k0]]] )?$builder[$_prefix.'max_date'][$_counter[$_elements[$k0]]]:"",
											'disabled_dates' 	=> isset( $builder[$_prefix.'disabled_dates'][$_counter[$_elements[$k0]]] )?$builder[$_prefix.'disabled_dates'][$_counter[$_elements[$k0]]]:"",
											'disabled_weekdays' => isset( $builder[$_prefix.'disabled_weekdays'][$_counter[$_elements[$k0]]] )?$builder[$_prefix.'disabled_weekdays'][$_counter[$_elements[$k0]]]:"",
				...
				if ( isset( $section['elements'] ) && is_array( $section['elements'] ) ) {
					...
					foreach ( $section['elements'] as $element ) {
						...
						$args = array(
							...
							'class_id'			=> "element_".$element_counter.$form_prefix,
