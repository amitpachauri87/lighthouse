	...
	public function wp_count_posts( $type = 'post', $perm = '' ) {
		...
		if( TM_EPO_WPML()->is_active() && TM_EPO_WPML()->get_lang()!='all' ){
			if ($_lang==TM_EPO_WPML()->get_default_lang()){
				$query .= " LEFT JOIN {$wpdb->postmeta} ON (p.ID = {$wpdb->postmeta}.post_id) LEFT JOIN {$wpdb->postmeta} AS mt1 ON (p.ID = mt1.post_id AND mt1.meta_key = 'tm_meta_lang')";
			}else{
		...
		if( TM_EPO_WPML()->is_active() && TM_EPO_WPML()->get_lang()!='all' ){			
			if ($_lang==TM_EPO_WPML()->get_default_lang()){
				$query .= " AND ( ( wp_postmeta.meta_key = 'tm_meta_lang' AND CAST(wp_postmeta.meta_value AS CHAR) = '".TM_EPO_WPML()->get_lang()."' ) OR mt1.post_id IS NULL ) ";
			}else{
				$query .= " AND pm.meta_key = 'tm_meta_lang' AND pm.meta_value = '".TM_EPO_WPML()->get_lang()."'";
			}
		}
