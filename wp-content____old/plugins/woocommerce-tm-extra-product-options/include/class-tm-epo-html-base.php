	...
	public function tm_make_field($args, $e=0) {
		...
		if (isset($args["type"])){
			...
			case "custom":
				...
				break;
			case "hidden":
				$a .="<input type='hidden' $tags />";
				break;
			case "text":
			case "number":
				$a .="<input ".(isset($args["style"])?$args["style"]:"")." type=\"text\" $tags />";
				break;
			case "range":
				$a .="<div class=\"rangewrapper\"><input type=\"text\" $tags /></div>";
				break;
	
			case "textarea":
				$a .="<textarea ".(isset($args["style"])?" ".$args["style"]:"")." $tags>".$args["default"]."</textarea>";
				break;
			case "checkbox":
				$sel=($args["default"]==$args["tags"]["value"])?" checked='checked'":"";
				$a .="<input type=\"checkbox\" $tags $sel />";
				break;
			case "radio":
				foreach ($args["options"] as $tx=>$vl){
					$sel=($args["default"]==$vl["value"])?" checked='checked'":"";
					$idd='id="'.$args["tags"]["id"].$tx.'" ';
					$a .="<label for='".$args["tags"]["id"].$tx."'><input ".$idd."type=\"radio\" name=\"".$args["tags"]["name"]."\" value=\"".$vl["value"]."\" $sel />&nbsp;".$vl["text"]."</label>";
					$a .="<br />";
				}
				break;
			case "select":
				$selectArray=array("class"=>isset($args["tags"]["class"])?$args["tags"]["class"]:"","id"=>$args["tags"]["id"],"name"=>$args["tags"]["name"],"extra"=>(isset($args["style"])?$args["style"]:"style=\"max-width:100%;min-width:20%;\""));
				...
				break;		
			case "select2":
				$selectArray=array("class"=>$args["tags"]["class"],"id"=>$args["tags"]["id"],"name"=>$args["tags"]["name"]);
				if(isset($args["multiple"])){
					$selectArray["multiple"]=$args["multiple"];
				}
				if(isset($args["size"])){
				...
				break;		
			case "select3":
				$selectArray=array("class"=>isset($args["tags"]["class"])?$args["tags"]["class"]:"","id"=>$args["tags"]["id"],"name"=>$args["tags"]["name"],"extra"=>(isset($args["style"])?$args["style"]:""));
				if(isset($args["multiple"])){
					$selectArray["multiple"]=$args["multiple"];
				}
				if(isset($args["size"])){
