...
class bkap_validation{
    ...
    */
    ...
    public static function bkap_get_bundle_item_validations( $product_id, $product ) {
        ...
        foreach ( $cart_configs as $cart_key => $cart_value ) {
            ...
            if ( isset( $cart_value['quantity'] ) && $cart_value['quantity'] > 0 ) {
                ...
                $quantity = bkap_validation::bkap_get_quantity( $cart_value['product_id'], $cart_value['quantity'] );
     ...
     *
     * @since   4.4.0
     * @global  object $wpdb Global wpdb Objcet
            ...
            if ( $type_of_slot == 'multiple' ) {
                ...
                if ( ! isset( $_POST[ 'validated' ] ) || ( isset( $_POST[ 'validated' ] ) && $_POST[ 'validated' ] == 'NO' ) ) {
                    ...
                    if ( $to_time != '' ) {
                        ...
                        if ( isset( $results ) && count( $results ) == 0 ) {
                            $f_t        = date( 'H:i', strtotime( $from_time ) );                            
                            $results    = $wpdb->get_results( $wpdb->prepare( $query, $post_id, $date_check, $f_t ) );
                        }

                        // If date record not found then search for base record
                        if ( isset( $results ) && count( $results ) == 0 ) {
                            ...
                            $query     =  "SELECT total_booking, available_booking, start_date FROM `".$wpdb->prefix."booking_history`
                                            ...
                                            AND from_time = %s
                                            AND status !=  'inactive'";
                            $results   =   $wpdb->get_results( $wpdb->prepare( $query, $post_id, $booking_weekday, $from_time ) );
                        }
                    }
