...
// Add a new interval for import cron schedule  
...
class bkap_calendar_sync {
                        ...
                        if ( ( isset( $booking_status ) && 'pending-confirmation' != $booking_status ) || ( ! isset( $booking_status )) ) {
                            ...
                            if ( $is_date_set ) {
                                ...
                                if ( isset( $_booking[ 'time_slot' ] ) && $_booking[ 'time_slot' ] != '' ) {
                                    ...
                                    if ( strpos( $_booking[ 'time_slot' ], "<br>" ) !== false ){
                                        $time_exploded = explode("<br>", $_booking[ 'time_slot' ]);
                                        array_shift( $time_exploded );
                                    }else{
                                        $time_exploded[] = $_booking[ 'time_slot' ];
                                    }
                                }
    ...
    /**
     * Returns an object which can be used to create 'Add to Calendar'
    ...
    /**
     * Creates 'Add to Google Calendar' & 'Add to Other Calendar'
     * buttons in WooCommerce emails
     *
     ...
     */
     ...
     public static function bkap_create_gcal_event_post( $bkap_event, $product_id = 0, $status, $option_name = '', $user_id = 1 ) {
        ...
        $new_booking_event = bkap_calendar_sync::get_bkap_booking( $new_event_data );  
