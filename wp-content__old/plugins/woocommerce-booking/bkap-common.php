...
class bkap_common{
    ...
    public static function ts_get_all_bookable_products(){
	...
	public static function bkap_check_timeslot_for_weekday( $product_id, $start_date ) {
        ...
        if( !isset( $booking_settings['booking_time_settings'] ) ){
            return false;
        }elseif ( is_array( $booking_settings['booking_time_settings'] ) && array_key_exists( $start_booking_weekday, $booking_settings['booking_time_settings'] ) ) {
            return true;
            ...
            foreach( $get_bundle_items as $b_key => $b_value ) {
                ...
                if( isset( $price_results ) && count( $price_results ) > 0 ) {
                    ...
                    if ( 'yes' == $price_type ) {
                        ...
                        }else if ( isset( $_POST['bundle_qty'][ $bundle_item_id ] ) ) {
                            $child_price = $child_price * $_POST['bundle_qty'][ $bundle_item_id ];
                        }
                ...
                if ( isset( $c_value['p_id'] ) && '' !== $c_value['p_id'] ) {
                    ...
                    if ( isset( $component_data['priced_individually'] ) && 'yes' === $component_data['priced_individually'] ) {
                        ...
                        if ( $child_type == 'bundle' ) {
                            $child_price = bkap_common::get_bundle_price( 0, $c_value['p_id'], $variation_id );
                        }else {
                            $child_price = bkap_common::bkap_get_price( $c_value['p_id'], $selected_variation, $child_type );
                        }
    ...
    public static function get_woocommerce_product_list( $variations = true, $bkap_option = "on" ) {
    ...
    /**
     * Echoing the text for the Privacy Policy Notice
     *
     * @since 4.9.0
     */

    public static function bkap_privacy_notice() {

       $privacy_notice = __( 'If you are exporting booking information to the Google Calendar using <b><a href="https://www.tychesoftwares.com/store/premium-plugins/woocommerce-booking-plugin/" target="_blank">Booking & Appointment for WooCommerce</a></b> plugin, then we will recommend you to mention about the customer data being exported to third party application from the plugin in your Privacy Policy as per the EU data protection laws through <b>General Data Protection Regulation</b> (GDPR).', "woocommerce-booking" );
