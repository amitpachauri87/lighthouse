...
if ( ! class_exists( 'BKAP_Bookings_View' ) ) {
	...
	class BKAP_Bookings_View {
		 ...
		 * 
		 * @param array $exisitng_columns Exiting Columns Array
		...
		function bkap_bulk_actions( $actions ) {
			...
			$actions['confirm_booking'] = __( 'Confirm Booking', 'confirm_booking');
			$actions['cancel_booking'] = __( 'Cancel Booking', 'cancel_booking');
		...
		function bkap_old_view() {
			...
			if ( 'edit.php' == $pagenow && $this->type == $post_type ) {
				...
				$message = __( 'Think some bookings are missing? If yes, please try checking for them', 'woocommerce-booking' );
                $message .= '<a href="' . get_admin_url() . $link . '">';
                $message .= __( 'here', 'woocommerce-booking' );
                $message .= '</a>';
