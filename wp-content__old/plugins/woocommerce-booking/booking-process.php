...
class bkap_booking_process {
    ...
    */
	...
	public static function bkap_before_add_to_cart() {
		...
		//         }
		//     } else {
		//         add_action( 'bkap_after_booking_box_form',    array( 'bkap_booking_process', 'bkap_price_display' ), 9 );
		//     }
		// }else{
		//     if ( is_plugin_active( 'bkap-deposits/deposits.php' ) && is_plugin_active( 'woocommerce-product-addons/woocommerce-product-addons.php' ) && is_plugin_active( 'gravityforms/gravityforms.php' )  ) {
	...
	public static function bkap_price_display() {
	    ...
	    $price_display = apply_filters( 'bkap_price_display_html', 
								    	'<div id="bkap-price-box">
								    		<div id="ajax_img"><img src="'. plugins_url() .'/woocommerce-booking/images/ajax-loader.gif"></div>
	...
	public static function bkap_localize_process_script( $post_id, $edit = false ) {
	    ...
	    $labels = apply_filters( "bkap_change_hover_text_for_disabled_dates", $labels );
	    ...
	    if( isset( $booking_settings[ 'booking_enable_time' ] ) && 'on' === $booking_settings[ 'booking_enable_time' ] ) { // if date_time booking method
        ...
        if( !empty( $resource_array ) ) {
        	...
        	$default_date = bkap_booking_process::bkap_first_available_resource_date( $product_id, $default_date );
            ...
            foreach( $resource_ids as $resource_id ) {
                ...
                if ( is_array( $resource_availability ) && count( $resource_availability ) > 0 ) {
	                    ...
	                    foreach ( $resource_availability as $key => $value ) {
	                        ...
	                        switch ( $value['type'] ) {
	                            ...
	                            case 'days':
	                                ...
	                                if ( strpos( $date_status, $date_day ) !== false && $value['bookable'] == 0 ) {
        ...
        if ( isset( $booking_settings['enable_inline_calendar'] ) && $booking_settings['enable_inline_calendar'] == 'on' ){
            ...
            if ( isset( $_SESSION['start_date'] ) && $_SESSION['start_date'] != '' ) {
                ...
                $hidden_date = bkap_booking_process::bkap_first_available_resource_date( $product_id, $hidden_date );
    ...
    */
	...
	/**
	 * Returns the checkout date for Multiple Day bookings that
	 * use Fixed Blocks along with Inline Calendar.
	 *
	 * This function is called on product page load.
	 * Called via AJAX
	 *
	 * @return string $date_final_checkout - Y-m-d - Checkout date.
	 * @since 2.5.4
	 */
    ...
    */
	...
	public static function bkap_first_available_resource_date( $product_id, $default_date ) {
		...
		if( is_array( $resource_ids ) && count($resource_ids) > 0 ) {
			$booking_placed_for_first_resource = bkap_calculate_bookings_for_resource( $resource_ids[0] );

		 	while( strpos( $booking_placed_for_first_resource['bkap_locked_dates'], $default_date ) !== false ) {
		 		$default_date = bkap_add_days_to_date( $default_date, 1 );
		 	}
		}        
