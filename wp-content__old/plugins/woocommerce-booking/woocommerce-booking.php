...
// setup the updater
$edd_updater = new EDD_BOOK_Plugin_Updater( EDD_SL_STORE_URL_BOOK, __FILE__,
											array(  'version' 	=> '4.9.0', 				// current version number
...
if ( !class_exists( 'woocommerce_booking' ) ) {
	...
	class woocommerce_booking {
		...
		public function __construct() {
			...
			add_action( 'bkap_after_booking_box_form', array( 'bkap_booking_process', 'bkap_price_display' ) );
		...
		public static function bkap_include_files() {
			...
			//Include files for privacy.			
			include_once( 'includes/class-bkap-privacy-policy.php' );
			include_once( 'includes/class-bkap-privacy-exporter.php' );
			include_once( 'includes/class-bkap-privacy-erasure.php' );
			...
			if ( function_exists( 'woo_vou_default_settings') ) {
				include_once( 'includes/class-bkap-wc-voucher-pdf.php' );
			}
			...
			if ( true === is_admin() ) {
				include_once( 'includes/bkap-all-component.php' );
			}
		}
		 ...
		 * @globals string $BookUpdateChecker Current Installed Version
		 * @globals mixed $wpdb Global wpdb object
		 */
		...
		function bkap_bookings_update_db_check() {
			...
			if ( $booking_plugin_version != $this->get_booking_version() ) {
				...
				update_option( 'woocommerce_booking_db_version', '4.9.0' );
		...
		public static function bkap_localize_view_booking() {
			...
			$bkap_view_booking['labels'] = array( 	'print_label' 	=> __( 'Print', 		'woocommerce-booking' ),
			...
			if ( current_user_can( 'operator_bookings' ) ) {
				$bkap_view_booking['url'] = array(	'print_url' 	=> esc_url( add_query_arg( array( 'download' => 'data.print' ) ) ),
		...
		static function inlcude_frontend_scripts_css( $product_id ){
			...
			if ( isset( $booking_settings[ 'booking_enable_date' ] ) && $booking_settings[ 'booking_enable_date' ] == 'on' ) {
				...
				wp_register_style( 'bkap-jquery-ui', plugins_url( "/woocommerce-booking/css/themes/$calendar_theme_sel/jquery-ui.css" ), '', $plugin_version_number, false );
		 ...
		 * 
		 * @param string $start_date Date for which number of Bookings needs to fetched
		 * @return mixed Booking Data
		 * @since 4.0.0
		 * @globals mixed $wpdb wpdb global variable
		 * @globals WP_Post $post Post Global Object
		 */
		 ...
		 * 
		 * @global mixed $wpdb wpdb global variable
		 */
		 ...
		 *
		 * @param array $global_settings Booking Global Settings
		 * @since 4.1.0
		 */
		...
		function bkap_init_post_types() {
			register_post_type( 'bkap_booking',
				apply_filters( 'bkap_register_post_type_bkap_booking',
					array(	'label'              	=> __( 'Booking', 'woocommerce-booking' ),
							'labels'             	=> array(	'name'               => __( 'Booking', 					'woocommerce-booking' ),
			...
			register_post_type( 'bkap_gcal_event', 
				apply_filters( 'bkap_register_post_type_bkap_gcal_event', array(
					'label'              => __( 'Import Bookings', 'woocommerce-booking' ),
					'labels'             => array(	'name'               => __( 'Google Event', 				'woocommerce-booking' ),
			...
			register_post_status( 'bkap-unmapped', array( 
			...
			register_post_status( 'bkap-mapped', array( 
			...
			register_post_status( 'bkap-deleted', array( 
			...
			register_post_type( 'bkap_resource',
				apply_filters( 'bkap_register_post_type_resource',
					array(
						...
						'labels' => array( 	'name'               => __( 'Bookable resource', 		'woocommerce-booking' ),
		...
		function bkap_meta_box_save( $post_data, $post ) {
			...
			if ( $qty_update || $date_update || $time_update ) {
				...
				if ( $order_id > 0 ) {
					...
					bkap_insert_event_to_gcal( $new_order_obj, $product_id, $item_id );
				}
				// add order notes
