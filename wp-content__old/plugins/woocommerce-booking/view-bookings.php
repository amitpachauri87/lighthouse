...
class view_bookings{
   ...
   public static function bkap_woocommerce_history_page() {
   	...
   	function generate_data( $tab_status ) {
		...
		if ( isset( $_GET[ 'bkap_start_date' ] ) && isset( $_GET[ 'bkap_end_date' ] ) ) {
   	    ...
   	    if ( $tab_status == 'future' ) {
		    ...
		    $booking_query   = "SELECT a1.post_id,a1.start_date,a1.end_date,a1.from_time,a1.to_time,a2.booking_id,a2.order_id FROM `".$wpdb->prefix."booking_history` AS a1,`".$wpdb->prefix."booking_order_history` AS a2 WHERE a1.id = a2.booking_id AND a1.start_date BETWEEN '".$start_date."' AND '".$end_date ."' ". $time_query . " AND a2.order_id IN ( SELECT ID FROM `" . $wpdb->prefix . "posts` WHERE post_type = 'shop_order' AND post_status NOT IN ('wc-cancelled', 'wc-refunded', 'trash', 'wc-failed', '') ) ORDER BY a2.order_id DESC";
	 ...
	 *
	 * @since 2.0
	 * @param array $report Array of all booking details
	 * @return string $csv Returns the strings which is created based on the booking details 
     */
   	...
   	function generate_csv( $report ) {
  		...
  		// Column Names
   		...
   		foreach ( $report as $key => $value ) {
   			...
   			$the_order          = wc_get_order( $value->order_id );
   			$currency           = ( version_compare( WOOCOMMERCE_VERSION, "3.0.0" ) < 0 ) ? $the_order->get_order_currency() : $the_order->get_currency();
   			$final_amt 			= strip_tags( html_entity_decode( wc_price( $amount, array( 'currency' => $currency ) ) ) );
   			
   			// Create the data row
   			$csv             .= $order_id . ',' . $customer_name . ',"' . $product_name . '","' . $checkin_date . '","' . $checkout_date . '","' . $time . '",' . $selected_quantity . ',' . $final_amt . ',' . $order_date;
   			$csv             .= "\n";  
   		}
