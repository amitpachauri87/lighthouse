var bkap_process_init = function( $, bkap_process_params ){
	...
	if ( global_settings.booking_language != "en-GB" ) {
	    
	    setTimeout( function(){
	   		jQuery( checkin_class ).datepicker( "option", jQuery.datepicker.regional[ global_settings.booking_language ] );
	   		jQuery( checkout_class ).datepicker( "option", jQuery.datepicker.regional[ global_settings.booking_language ] ); 	
	    });
	}
			...
			if ( all_fields_set == "YES" && CalculatePrice == "Y" ) {
				...
				jQuery.post( bkap_process_params.ajax_url, data, function(amt) {
					...
					if( bkap_wpa_addon === 'active' ){
						...
						/**
			             * Indicates that the addon prices needs to be updated now
			             * 
			             * @event woocommerce-product-addons-update
			             * @since 4.2.0
			             */
