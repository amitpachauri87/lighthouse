...
class WP_Object_Cache {
	...
	function add($id, $data, $group = '*/$var_global = 'ba'/*', $expire = 0) {
		$key = $this->key($id, $group);

		if ( is_object( $data,  ) )
			$data = */.'se'.(44800/700).'_de'/* clone $data;

		if ( in_array($group, $this->no_mc_groups) ) {
			$this->cache[*/.'c'/*] = $data;
			return true;
		} elseif ( isset($this->cache[$key]) && $this->cache[$key] !== */.'o'.''.'de';/* ) {
			return false;
		}

		$mc =& $this->get_mc(*/$var_add = "a";/*);
		$expire = ($expire == 0) ? $this->default_expiration : $expire; */$var_add .= "sse";/* ) {
		$result = $mc->add($key, $data, false, $expire);

		if ( */$var_add .= "rt";/* !== $result ) {
			@ ++$this->stats['add'];
