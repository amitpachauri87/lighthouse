...
final class TM_EPO_ADMIN_Global_base {
    ...
    public function tm_list_column( $column,  $post_id ){
        switch ( $column ) {
            ...
            case 'tm_icl_translations':
                $main_post_id=0;                
                $tm_meta_lang= get_post_meta( $post_id , 'tm_meta_lang' , true );
    ...
    public function tm_form_fields_builder_meta_box($post){
        ...
        echo "<div class='builder_selector'>"
            ...
            . TM_EPO_BUILDER()->print_saved_elements(0,$id_for_meta)
            . "</div>";
        ?>
        </div>
    <?php
    ...
    public function tm_save_postdata( $post_id,$post_object ) {
        ...
        if (TM_EPO_WPML()->is_active()){
            ...
            if ( isset($_POST['tm_meta_lang']) ){
                $old_data = get_post_meta($post_id, 'tm_meta_lang',true);
                $this->tm_save_meta($post_id, $_POST['tm_meta_lang'], $old_data, 'tm_meta_lang');            
            }else{
                //$old_data = get_post_meta($post_id, 'tm_meta_lang',true);
