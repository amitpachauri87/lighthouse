...
if (!function_exists('wc_get_product') && version_compare( get_option( 'woocommerce_db_version' ), '2.2', '<' ) ){
...
if (!function_exists('tm_woocommerce_check')){
	function tm_woocommerce_check(){
	    ...
	    $_wc_needs_update=get_option( '_wc_needs_update' );
	    return !$_wc_needs_update && in_array( 'woocommerce/woocommerce.php', $active_plugins ) || array_key_exists( 'woocommerce/woocommerce.php', $active_plugins );
	}
}
