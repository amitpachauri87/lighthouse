...
final class TM_EPO_BUILDER_base {
	...
	private function init_elements(){
		...
		$this->elements_array=array(
			...
			"textarea"=>$this->add_element(
				"textarea",
				array("required","price","text_after_price","price_type","hide_amount","placeholder","max_chars")
				),			
			
			"textfield"=>$this->add_element(
				"textfield",
				array("required","price","text_after_price","price_type2","hide_amount","placeholder","max_chars")
				),
			
			"selectbox"=>$this->add_element(
				"selectbox",
				array("required","text_after_price",($this->woo_subscriptions_check)?"price_type3":"price_type4","hide_amount","placeholder","use_url","changes_product_image","options")
				),			
			
			"radiobuttons"=>$this->add_element(
				"radiobuttons",
				array("required","text_after_price","hide_amount","use_url","use_images","changes_product_image","swatchmode","items_per_row","options")
				),			

			"checkboxes"=>$this->add_element(
				"checkboxes",
				array("required","text_after_price","hide_amount","limit_choices","exactlimit_choices","use_images","changes_product_image","swatchmode","items_per_row","options")
				),			
			...
			"date"=>$this->add_element(
				"date",
				array("required","price","text_after_price","price_type6","hide_amount","button_type2","date_format","start_year","end_year",
					array(
						"id" 				=> "date_min_date",
						"default"			=> "",
						"type"				=> "text",
			...
			"range"=>$this->add_element(
				"range",
				array("required","price","text_after_price","price_type7","hide_amount","min","max",
					array(
						"id" 		=> "range_step",
						"default"	=> "1",
						"type"		=> "text",
			...
			"color"=>$this->add_element(
				"color",
				array("required","price","text_after_price","price_type6","hide_amount","default_value")
				),
	...
	public function print_elements( $echo=0 ) {
		...
		foreach ( $this->get_elements() as $element=>$settings ) {
			if ( isset( $this->elements_array[$element] ) ) {
				if(empty($settings['no_selection'])){
					$drag_elements .="<div data-element='".$element."' class='ditem element-".$element."'><div class='label'><i class='tmfa fa ".$settings["icon"]."'></i> ".$settings["name"]."</div></div>";
				}
				...
				$out1 	.="<div class='bitem element-".$element." ".$settings["width"]."'>"
						...
						."<div class='hstc2 closed'><div class='icon fa fa-sort move'></div>"
						."<div class='icon fa fa-minus minus'></div><div class='icon fa fa-plus plus'></div>"
						."<div class='icon size'>".$settings["width_display"]."</div>"
						."<div class='icon fa fa-pencil edit'></div><div class='icon fa fa-copy clone'></div><div class='icon fa fa-times delete'></div>"
						."<div class='label-icon'><i class='tmfa fa ".$settings["icon"]."'></i></div>"
						."<div class='label'>".$settings["name"]."</div><div class='inside'><div class='manager'>"
		...
		$out  ='<div class="builder_elements"><div class="builder_hidden_elements" data-template="'.esc_html( json_encode( array( "html"=>$out1 ) ) ).'"></div>'
				...
				.'<div class="builder_actions">'.'<a class="builder_add_section tm-button bsbb" href="#"><i class="fa fa-plus-square"></i> '.__("Add section",TM_EPO_TRANSLATION).'</a>'.'</div>'
				."</div>";
		if ( empty( $echo ) ) {
			return $out;
		}else {
	...
	public function section_elements( $echo=0 ) {
		...
		$out= "<div class='builder_wrapper'>"
			...
			. "</div>"
			. "<div class='btitle'>"
			. "<div class='icon fa fa-sort move'></div><div class='icon fa fa-minus minus'></div><div class='icon fa fa-plus plus'></div>"
		...
		if ( !empty( $post_id ) && is_array( $builder ) && count( $builder )>0 && isset($builder['element_type']) && is_array($builder['element_type']) && count($builder['element_type'])>0 ) {
			...
			for ( $_s = 0; $_s < count( $_sections ); $_s++ ) {
				...
				foreach ( $this->_section_elements as $_sk=>$_sv ) {
					...
					$out .=TM_EPO_HTML()->tm_make_field( $_sv, 0 );
				}
					...
					if (isset($_elements[$k0])){
						if ( isset( $this->elements_array[$_elements[$k0]] ) ) {
							$out .="<div class='bitem element-".$_elements[$k0]." ".$_div_size[$k0]. "'>"
								 ...
								 . "<div class='hstc2 closed'><div class='icon fa fa-sort move'></div><div class='icon fa fa-minus minus'></div><div class='icon fa fa-plus plus'></div>"
								 . "<div class='icon size'>". $this->sizer[$_div_size[$k0]]."</div>"
								 . "<div class='icon fa fa-pencil edit'></div><div class='icon fa fa-copy clone'></div><div class='icon fa fa-times delete'></div>"
								 . "<div class='label-icon'><i class='tmfa fa ".$_this_elements[$_elements[$k0]]["icon"]."'></i></div>"
								 . "<div class='label'>".$_this_elements[$_elements[$k0]]["name"]."</div><div class='inside'><div class='manager'>";
							...
							foreach ( $_temp_option  as $key=>$value ) {
								if ( isset( $value['id'] ) ) {
									...
									if (in_array($value['id'],array("checkboxes_options","radiobuttons_options","selectbox_options"))){
										...
										if (is_null($t[$_vid])){
											...
											$_imagesp=isset($builder['multiple_'.$value['id'].'_imagep'])?$builder['multiple_'.$value['id'].'_imagep']:null;
											...
											if (!is_null($_titles) && !is_null($_values) && !is_null($_prices) ){
												...
												if (is_null($_imagesp)){
													$_imagesp=$_titles;
													$_imagesp = array_map(array( $this,'_tm_clear_array_values'), $_imagesp);
												}
												...
												foreach ($_titles as $option_key=>$option_value){
													$t[$_vid][]=array(
														"title" => $_titles[$option_key],
														"value" => $_values[$option_key],
														"price" => $_prices[$option_key],
														"image" => $_images[$option_key],
														"imagep" => $_imagesp[$option_key],
														"price_type" => $_prices_type[$option_key],
														"url" => $_url[$option_key]
													);
												}
											}
								...
								$out .=TM_EPO_HTML()->tm_make_field( $value, 0 );
							}
	...
	public function builder_sub_variations_options( $meta=array(), $product_id=0 ) {
		...
		foreach ($attributes as $name => $options) {
			$out .=   '<div class="tma-handle-wrap tm-attribute">'
					. '<div class="tma-handle"><div class="tma-attribute_label">'.wc_attribute_label($name).'</div><div class="icon fa fold fa-caret-up"></div></div>'
			...
			if ( is_array( $options ) ) {
				...
				if ( taxonomy_exists( sanitize_title( $name ) ) ) {
					...
					if (!empty($args)){
						...
						foreach ( $terms as $term ) {
							...
							$out .=   '<div class="tma-handle-wrap tm-term">'
									. '<div class="tma-handle"><div class="tma-attribute_label">'.apply_filters( 'woocommerce_variation_option_name', $term->name ) .'</div><div class="icon fa fold fa-caret-up"></div></div>'
									. '<div class="tma-handle-wrapper tm-hidden">'
									. '<div class="tma-attribute w100">';
					...
					foreach ( $options as $option ) {
						$out .=   '<div class="tma-handle-wrap tm-term">'
								. '<div class="tma-handle"><div class="tma-attribute_label">'.esc_html( apply_filters( 'woocommerce_variation_option_name', $option ) ) .'</div><div class="icon fa fold fa-caret-up"></div></div>'
								. '<div class="tma-handle-wrapper tm-hidden">'
								. '<div class="tma-attribute w100">';
